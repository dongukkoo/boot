...
public interface Mapper01 {
	...
	@Select("""
			SELECT SupplierID id, SupplierName name, contactName, address, city, country, postalCode, phone
			FROM Suppliers
			WHERE SupplierID = 2
			""")
	Supplier method13();
	...
	@Select("""
			SELECT EmployeeID id ,LastName, FirstName, BirthDate birth, Photo, Notes
			FROM Employees
			WHERE EmployeeID = 5
			""")
	Employee method14();
